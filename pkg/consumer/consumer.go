...
func (c *consumer) AdvanceLSN(lsn dbutils.LSN) {
	c.currentLSN = lsn
}
	...
	for {
		...
		default:
			...
			if repMsg.WalMessage != nil {
				...
				if err := handler.HandleMessage(msg, dbutils.LSN(repMsg.WalMessage.WalStart)); err != nil {
...
func (c *consumer) SendStatus() error {
	//log.Printf("sending status: %v", c.currentLSN)
